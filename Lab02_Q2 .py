...
def EM_Trap(func_firstorder,N,a,b):
    ...
    # Use EM method to estimate error
...
'''   
#define the integral target 
def sample(t):
    return np.exp(-t**2)
#define the integral target first order derivitive
def sample_firstorder(t):
    return -2*np.exp(-t**2)*t
#define the integral target third order derivitive
def sample_thirdorder(t):
    return -4*np.exp(-t**2)*t*(2*t**2-3)

print(2/np.sqrt(np.pi)*Trap(sample,10,0,3),'\n',2/np.sqrt(np.pi)*Simp(sample,10,0,3),'\n',erf(3))
...
error_Trap = 1./3 * (Trap(sample,10*2,0,3) - Trap(sample,10,0,3))
...
error_Simp = 1./15 * (Simp(sample,10*2,0,3) - Simp(sample,10,0,3))
